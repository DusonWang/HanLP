...
//                    continue;
     ...
     * @return 处理后的列表
     */
    private static List<Item> normalizeFrequency(List<Item> itemList) {
        for (Item item : itemList) {
            ArrayList<Map.Entry<String, Integer>> entryArray = new ArrayList<Map.Entry<String, Integer>>(item.labelMap.entrySet());
            entryArray.sort((o1, o2) -> o1.getValue().compareTo(o2.getValue()));
            int index = 1;
     ...
     * @param itemList
     */
    private void addAllNotCombine(List<Item> itemList) {
        for (Item item : itemList) {
            addNotCombine(item);
        }
    }
     ...
     * @param item
     */
    private void addNotCombine(Item item) {
        Item innerItem = trie.get(item.key);
        if (innerItem == null) {
            innerItem = item;
            trie.put(innerItem.key, innerItem);
