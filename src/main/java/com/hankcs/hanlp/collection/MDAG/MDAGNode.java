...
public class MDAGNode {
    ...
    public MDAGNode transition(char[] str) {
        ...
        //Iteratively _transition through the MDAG using the chars in str
        for (char aStr : str) {
            currentNode = currentNode.transition(aStr);
            if (currentNode == null) break;
        }
        /////
...
//        sb.append(", storedHashCode=").append(storedHashCode);
        return "MDAGNode{" + "isAcceptNode=" + isAcceptNode +
                ", outgoingTransitionTreeMap=" + outgoingTransitionTreeMap.keySet() +
                ", incomingTransitionCount=" + incomingTransitionCount +
                '}';
    }
}
