...
public class CRFModel implements ICacheAble {
    ...
    public static CRFModel loadTxt(String path, CRFModel instance) {
        // 先尝试从bin加载
        if (instance.load(ByteArray.createByteArray(path + Predefine.BIN_EXT))) return instance;
        ...
        String line;
        int id = 0;
        instance.tag2id = new HashMap<String, Integer>();
        while ((line = lineIterator.next()).length() != 0) {
            instance.tag2id.put(line, id);
            ++id;
        }
        instance.id2tag = new String[instance.tag2id.size()];
        final int size = instance.id2tag.length;
        for (Map.Entry<String, Integer> entry : instance.tag2id.entrySet()) {
            instance.id2tag[entry.getValue()] = entry.getKey();
        }
        ...
        instance.featureTemplateList = new LinkedList<FeatureTemplate>();
        while ((line = lineIterator.next()).length() != 0) {
            if (!"B".equals(line)) {
                FeatureTemplate featureTemplate = FeatureTemplate.create(line);
                instance.featureTemplateList.add(featureTemplate);
            } else {
                instance.matrix = new double[size][size];
            }
        }
        ...
        if (instance.matrix != null) {
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
                    instance.matrix[i][j] = Double.parseDouble(lineIterator.next());
                }
            }
        }
        ...
        instance.featureFunctionTrie.build(featureFunctionMap);
        // 缓存bin
        try {
     ...
     * @return
     */
    private static CRFModel loadBin(String path) {
     ...
     * @return
     */
    protected Integer getTagId(String tag) {
