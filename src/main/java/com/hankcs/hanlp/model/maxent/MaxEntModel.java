...
public class MaxEntModel {
     ...
     */
    private DoubleArrayTrie<Integer> pmap;
     ...
     * @return
     */
    private static double[] eval(int[] context, double[] prior, EvalParameters model) {
        Context[] params = model.getParams();
        int numfeats[] = new int[model.getNumOutcomes()];
        int[] activeOutcomes;
        double[] activeParameters;
        double value = 1;
        for (int aContext : context) {
            if (aContext >= 0) {
                Context predParams = params[aContext];
    ...
    public static MaxEntModel create(String path) {
        ...
        try {
            ...
            int pid = 0;
            for (int[] outcomePattern1 : outcomePatterns) {
                int[] outcomePattern = new int[outcomePattern1.length - 1];
                System.arraycopy(outcomePattern1, 1, outcomePattern, 0, outcomePattern1.length - 1);
                for (int j = 0; j < outcomePattern1[0]; j++) {
                    double[] contextParameters = new double[outcomePattern1.length - 1];
                    for (int k = 1; k < outcomePattern1.length; k++) {
     ...
     * @return 概率数组
     */
    private final double[] eval(String[] context) {
     ...
     * @return 概率数组
     */
    private double[] eval(String[] context, double[] outsums) {
        assert context != null;
